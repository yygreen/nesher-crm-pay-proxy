...
| `draft.js`, `quote.js`, `mercury.js`, `nmi-card.js`, `nmi-webhook.js`, `invoice-*.js`, `open-pay.js`, `payments-sync.js` | Invoice drafting, Mercury AR API, Pinpoint/NMI CRM-priced card mint, guest pay page `/pay/<slug>`, Nesher open-amount `/pay/open`, signed NMI sale webhook `/__nesher_pay/nmi-webhook`, paid-invoice sync. |
