...
describe("confirmed NMI payment SQL integration", () => {
  ...
  it("reviews a legacy NMI marker found in the other CRM payment table", async () => {
    ...
    assert.match(hotelNotes[0].note, /received and NOT recorded: the same card transaction is already on another booking/);
  ...
  it("reviews a hotel payment whose offer belongs to no matching request", async () => {
    ...
    assert.match(offerNotes[0].note, /received and NOT recorded: the hotel offer belongs to another request/);
    assert.deepEqual(
