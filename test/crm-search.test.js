...
import { createCrmSearch, parseQuery, maskDigits, isReadOnlySql, shape } from "../crm-search.js";
...
test("isReadOnlySql refuses every write", () => {
  ...
  for (const s of ["UPDATE core_customer SET full_name='x'", "SELECT 1; DELETE FROM core_payment", "WITH x AS (DELETE FROM a) SELECT 1", "COPY core_customer TO '/tmp/x'"]) assert.equal(isReadOnlySql(s), false, s);
});
...
test("search by name: customer, traveller, bookings, payments in the window, refunds - Nesher and JRM apart", async () => {
  ...
  const b = r.body;
  assert.deepEqual(b.nesher.customers.map((c) => c.name), ["Moshe Cohen"]);
  assert.deepEqual(b.nesher.travelers, [{ name: "Rivka Cohen", booking: "79RHW4" }]);
  assert.deepEqual(b.nesher.bookings.map((x) => x.booking).sort(), ["79RHW4", "ABC123"]);
  const bk = b.nesher.bookings.find((x) => x.booking === "79RHW4");
  assert.equal(bk.balance_usd, 1000);
  // the window keeps 2026 only: the 2025 check is out
  assert.deepEqual(b.nesher.payments.map((p) => p.amount_usd).sort((a, z) => a - z), [400, 1000]);
  assert.equal(b.nesher.payments_total_usd, 1400);
  assert.equal(b.nesher.payments.find((p) => p.amount_usd === 1000).processor_txn, "12558953073");
  assert.equal(b.nesher.refunds.length, 1);
  assert.equal(b.nesher.customer_level_payments[0].imported_from_booking_payments, true);
...
test("search by Israeli phone finds the customer and her JRM request, currencies kept apart", async () => {
  ...
  assert.equal(r.status, 200);
  assert.deepEqual(r.body.nesher.customers.map((c) => c.name), ["Yael Sher"]);
  assert.deepEqual(r.body.jrm.hotel_requests.map((h) => h.request), ["JRM-325"]);
  assert.deepEqual(r.body.jrm.payments_total_by_currency, { ILS: 3000, USD: 500 });
  assert.equal(r.body.jrm.payments.find((p) => p.currency === "ILS").card_last4, "4421");
});
...
test("search by booking code and by JRM request id", async () => {
  ...
  assert.deepEqual(r.body.nesher.bookings.map((x) => x.booking), ["79RHW4"]);
...
test("the transaction is read only and rolled back: a write inside it would fail", async () => {
  ...
  assert.ok(seen.every((w) => ["BEGIN", "SELECT", "ROLLBACK"].includes(w)), seen.join(","));
});
